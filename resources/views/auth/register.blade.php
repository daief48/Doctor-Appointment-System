<x-guest-layout>
    <x-authentication-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="flex items-center justify-end mt-4" style="display: flex;justify-content: space-between;margin-top: 28px;">
                <a class="underline text-sm text-gray-600 hover:text-gray-900 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" href="{{ route('login') }}">
                    {{ __('Already registered Now Log In?') }}
                </a>

                <x-button class="ml-4" style="background: #36B7B4">
                    {{ __('Register') }}
                </x-button>
            </div>
        </form>
